use super::{CombatStats, GameLog, Map, Name, Player, Position, BASE_LIGHT_LEVEL};
use bracket_terminal::prelude::{BTerm, RGB};
use specs::prelude::*;
...
pub fn draw_ui(world: &World, ctx: &mut BTerm) {
    ...
    for msg in log.entries.iter().rev() {
        if y < 49 {
            ctx.print_color(2, y, RGB::from_f32(0.8, 0.8, 0.8), RGB::from_f32(0.0, 0.0, 0.0), msg);
        }
        y += 1;
    }
